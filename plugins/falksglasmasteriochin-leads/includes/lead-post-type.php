...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'falksglasmasteriochin-leads' ),
					'singular_name' => __( 'Lead', 'falksglasmasteriochin-leads' ),
					'edit_item'     => __( 'Förfrågan', 'falksglasmasteriochin-leads' ),
					'search_items'  => __( 'Search leads', 'falksglasmasteriochin-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'falksglasmasteriochin-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'falksglasmasteriochin-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'falksglasmasteriochin-leads' ),
			'npl_phone'    => __( 'Phone', 'falksglasmasteriochin-leads' ),
			'npl_service'  => __( 'Needs', 'falksglasmasteriochin-leads' ),
			'npl_timeline' => __( 'When', 'falksglasmasteriochin-leads' ),
			'npl_zip'      => __( 'ZIP', 'falksglasmasteriochin-leads' ),
			'npl_source'   => __( 'Source', 'falksglasmasteriochin-leads' ),
			'npl_status'   => __( 'Status', 'falksglasmasteriochin-leads' ),
			'date'         => __( 'Received', 'falksglasmasteriochin-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'falksglasmasteriochin-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'falksglasmasteriochin-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'falksglasmasteriochin-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'falksglasmasteriochin-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'falksglasmasteriochin-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'falksglasmasteriochin-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'falksglasmasteriochin-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'falksglasmasteriochin-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'falksglasmasteriochin-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'falksglasmasteriochin-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'falksglasmasteriochin-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'falksglasmasteriochin-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
