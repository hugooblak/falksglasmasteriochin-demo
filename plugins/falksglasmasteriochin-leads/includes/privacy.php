...
add_filter(
	...
	function ( $exporters ) {
		$exporters['falksglasmasteriochin-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'falksglasmasteriochin-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'falksglasmasteriochin-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'falksglasmasteriochin-leads',
						'group_label' => __( 'Quote requests', 'falksglasmasteriochin-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['falksglasmasteriochin-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'falksglasmasteriochin-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
